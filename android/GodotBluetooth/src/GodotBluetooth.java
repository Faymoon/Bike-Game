...
{   
							...
							activity.runOnUiThread(
								new Runnable()
								{
									...
									public void run()
									{
										...
										{
											Log.d("MyApp", "data received");
											GodotLib.calldeferred(instanceId, "_on_data_received", new Object[]{ msg.obj });
										}
									}
								});
	...
	public void getPairedDevices(final boolean nativeDialog) {
		if (initialized) {
			activity.runOnUiThread(new Runnable() {
				@Override
				public void run() {
					Log.d("MyApp", "start get paired devices");
					if(connected) {
							try {
								socket.close();
								connected = false;
					...
					Log.d("MyApp", "end get paired devices");
				}
			});
		}
	...
	private void nativeLayoutDialogBox() {
		Log.d("MyApp", "start nativeLayoutDialogBox");
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(activity);
			...
			dialog.show();
		Log.d("MyApp", "end nativeLayoutDialogBox");
	}
	...
	public void listPairedDevices() {
		...
		if(pairedDevices.size() > 0) {
			...
			for (BluetoothDevice device : pairedDevices) {
				...
				Log.d("MyApp", externalDeviceName + " MAC : " + externalDeviceAddress);
				...
				@Override
					public void run() {
						if(!connected){
							...
							Log.d("MyApp", "trying to connect 2");
							macAdress = device.getAddress();
							remoteBluetoothName = device.getName();

							Log.d("MyApp", remoteBluetoothName);
		...
		public void run() {
			...
			while (true) {
				try {
					...
					for (int i = 0; i < 10; ++i)
					{
						Log.d("MyApp", Integer.toString(buffer[i]));
					}
					//String externalData = new String(buffer, 0, bytes);
					localHandler.obtainMessage(MESSAGE_READ, bytes, -1, buffer).sendToTarget();
				} 
